...
class UDPServer:
    ...
    def new_item(self, item_id, description, minimum, ownername):
        new_bidding_item = Item(item_id, description, minimum, ownername)
    ...
    def offer(self, sender_info, data_packet):
        ...
        if status[0] is not True:
            ...
            self.send(cPickle.dumps(response), sender_info)
            self.new_item(status[0], status[1], status[2], sender_name)
