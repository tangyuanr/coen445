...
#####################################
    ...
    def startSubWindow(self, name, title=None, stopfunc=None, threadfunc = None, threadArgs = None, modal=False, blocking=False, transient=False, grouped=True):
        self.widgetManager.verify(self.Widgets.SubWindow, name)
        gui.trace("Starting subWindow %s", name)

        if stopfunc is None:
            top = SubWindow(self, self.topLevel, name, title=title, stopFunc = self.confirmHideSubWindow, threadFunc = threadfunc,
                            threadArgs=threadArgs, modal=modal, blocking=blocking, transient=transient, grouped=grouped)
        else:
            top = SubWindow(self, self.topLevel, name, title=title, stopFunc = stopfunc, threadFunc = threadfunc,
                            threadArgs=threadArgs, modal=modal, blocking=blocking, transient=transient, grouped=grouped)
        if threadfunc is not None:
            top.startThread()
...
class SubWindow(Toplevel, object):
    def __init__(self, win, parent, name, title=None, stopFunc=None, threadFunc = None, threadArgs = None, modal=False, blocking=False, transient=False, grouped=True, *args, **kwargs):
    ...
    def startThread(self):  # TODO: THREADGIN!
        self._loadThreading()
        if Queue is False:
            gui.warn("Unable to queueFunction - threading not possible.")
        else:
            t = Thread(group=None, target=self.threadFunc, name=None, kwargs=self.args)
            t.daemon = True
            t.start()
    ...
    def hide(self, useStopFunction=True):
        if useStopFunction:
